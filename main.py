...
# Set timestamp as index so plots use real time on x-axis
data = df.set_index('timestamp')[cols_to_scale]
...
axes[0].plot(data.index, data[cpu], label=cpu)
axes[0].plot(data.index, data[ram], label=ram)
axes[0].set_title("CPU and RAM Usage")
axes[0].legend()
axes[0].grid(True)

axes[1].plot(data.index, data[cpu], label="CPU original")
axes[1].plot(data.index, data[cpu + "_trend"], label="CPU trend")
axes[1].set_title("CPU Trend")
axes[1].legend()
axes[1].grid(True)

axes[2].plot(data.index, data[ram], label="RAM original")
axes[2].plot(data.index, data[ram + "_trend"], label="RAM trend")
axes[2].set_title("RAM Trend")
axes[2].set_xlabel("Time (seconds from cluster start)")
axes[2].legend()
axes[2].grid(True)
